...
@section('layout')
    ...
    <div>
        <div class="h-screen flex justify-center items-center">
            <form action="{{ route('dashboard.update', $dashboard->id) }}" method="POST" class="w-fit h-2/4 mx-auto pt-10 grid grid-cols-2 gap-3">
                @csrf
                @method('PUT')
                <label class="text-white text-xl">Name</label>
                <input name="name" type="text" placeholder="Name" required class="rounded-md px-2" value="{{ $dashboard->name }}">
                <label class="text-white text-xl">Value</label>
                <textarea name="value" type="textarea" placeholder="Value" required class="rounded-md px-2 pt-2">{{ $dashboard->value }}</textarea>
