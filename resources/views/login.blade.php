...
<html lang="en">
    ...
    <body class="bg-slate-800 grid place-items-center h-screen">
        <form class="bg-slate-700 w-fit p-5 rounded-md mx-auto" method="POST" action="{{ route('login.admin') }}">
            @csrf
            @method('POST')
            <div class="w-full mx-auto flex flex-col gap-5">
                <h1 class="text-xl text-white w-fit mx-auto">Login</h1>
                <div>
                    <h1 class="text-white w-fit mb-2">Name</h1>
