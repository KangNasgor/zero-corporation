        ...
        <div class="w-10/12 mx-auto mt-28 flex flex-col gap-3 items-end">
            <div class="flex items-center gap-4">
                <div class="flex gap-3 w-10/12 mx-auto pl-4">
                    <a href="{{ route('aboutimg.create') }}">
                        <button class="text-white bg-slate-600 py-2 px-3 rounded-md">
                            Create
                        </button>
                    </a>
                    <a href="{{ route('about.image') }}">
                        <button class="text-white bg-slate-600 py-2 px-3 rounded-md">
                            Home
                        </button>
                    </a>
                </div>
                <form method="GET" action="{{ route('about.image') }}" class="flex items-center">
                ...
                <tbody> 
                    ...
                    @elseif($content)
                        @foreach ($content as $ct)
                            <tr>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">{{ $ct->id }}</td>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">
                                    <img src="{{ asset('storage/' . $ct->image) }}" alt="img" class="w-16 h-16 object-cover">
                                </td>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">{{ $ct->name }}</td>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">{{ $ct->role }}</td>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">{{ $ct->status }}</td>
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-200 flex gap-2">
                                    <form action="{{ route('aboutimg.restore', $ct->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <a>
                                        <button class="bg-red-600 py-2 px-3 rounded-md delete-button" data-id="{{ $ct->id }}">
                                            Delete
                                        </button>
                                        <div class="bg-black/75 w-full h-full hidden justify-center items-center fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50"
                                            ...
                                            <div class="h-1/4 w-1/4 p-5 flex flex-col justify-between bg-slate-500 rounded-md">
                                                <div class="flex items-center justify-between">
                                                    <h1 class="text-xl">Delete "{{ $ct->name }}"?</h1>
                                                    <div class="flex flex-col gap-px cursor-pointer p-1 close-modal" data-id="{{ $ct->id }}">
                                                        <div class="bg-slate-800 h-px w-3 -rotate-45 translate-y-px"></div>
                                                        <div class="bg-slate-800 h-px w-3 rotate-45 -translate-y-px"></div>
                                                    </div>
                                                </div>
                                                <div class="flex gap-2 w-full justify-evenly">
                                                    <form action="{{ route('aboutimg.delete', $ct->id) }}" method="POST" class="w-fit bg-red-600 p-2 rounded-md">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit">
                                                            Delete
                                                    ...
                                                    <button class="w-fit bg-teal-400 text-black p-2 rounded-md close-modal" data-id="{{ $ct->id }}">
                                                        Cancel
                                                    </button>
                                                </div>
                                            </div>
