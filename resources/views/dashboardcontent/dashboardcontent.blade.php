        ...
        <div class="w-10/12 mx-auto mt-28 flex flex-col gap-3 items-end">
            <div class="flex items-center gap-4">
                <div class="flex gap-3 w-10/12 mx-auto pl-4">
                    ...
                    <a href="{{ route('dashboardcontent.history') }}">
                        <button class="text-white bg-slate-600 py-2 px-3 rounded-md">
                            History
                        </button>
                    </a>
                    ...
                    @elseif($dashboardcontent)
                        @foreach ($dashboardcontent as $content)
                            <tr>
                                ...
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-200 flex gap-3">
                                    <a href="{{ route('dashboardcontent.editpage', $content->id) }}">
                                    ...
                                    <a>
                                        ...
                                        <div class="bg-black/75 w-full h-full hidden justify-center items-center fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50" id="modal-{{ $content->id }}">
                                            <div class="h-1/4 w-1/4 p-5 flex flex-col justify-between bg-slate-500 rounded-md">
                                                <div class="flex items-center justify-between">
                                                    <h1 class="text-xl">Delete "{{ $content->product->name }}"?</h1>
                                                ...
                                                <div class="flex gap-2 w-full justify-evenly">
                                                    <form action="{{ route('dashboardcontent.softdelete', $content->id) }}"
