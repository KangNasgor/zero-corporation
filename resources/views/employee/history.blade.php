                        ...
                        @foreach ($employee as $emp)
                            <tr>
                                ...
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-200 flex gap-2 w-fit">
                                    <form action="{{ route('employee.restore', $emp->id) }}" method="POST" class="w-fit">
                                        @csrf
                                        @method('PUT')
                                    ...
                                    <a>
                                        ...
                                        <div class="bg-black/75 w-full h-full hidden justify-center items-center fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 modal" id="modal-{{ $emp->id }}">
                                            <div class="h-1/4 w-1/4 p-5 flex flex-col justify-between bg-slate-500 rounded-md">
                                                <div class="flex items-center justify-between">
                                                    <h1 class="text-xl">Delete "{{ $emp->name }}" permanently?</h1>
                                                ...
                                                <div class="flex gap-2 w-full justify-evenly">
                                                    <form action="{{ route('employee.delete', $emp->id) }}"
