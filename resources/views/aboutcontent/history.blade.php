        ...
        <div class="w-10/12 mx-auto mt-28 flex flex-col gap-3 items-end">
            <div class="flex items-center gap-4">
                <div class="flex gap-3 w-10/12 mx-auto pl-4">
                    <a href="{{ route('aboutcontent.create') }}">
                ...
                <form method="GET" action="{{ route('aboutcontent') }}" class="flex items-center">
            ...
            <table class="table-auto border-collapse w-full rounded-md text-sm bg-slate-800 text-slate-200">
                <thead>
                    <tr>
                        ...
                        <th class="border-slate-600 font-medium p-4 pl-8 pt-3 pb-3 text-left">Value</th>
                    ...
                    @elseif($content)
                        @foreach ($content as $ct)
                            <tr>
                                ...
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-400">{{ $ct->value }}</td>
                                ...
                                <td class="border-b border-slate-700 p-4 pl-8 text-slate-200 flex gap-2">
                                    <form action="{{ route('aboutcontent.restore', $ct->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <div class="bg-black/75 w-full h-full hidden justify-center items-center fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50"
                                            ...
                                            <div class="h-1/4 w-1/4 p-5 flex flex-col justify-between bg-slate-500 rounded-md">
                                                ...
                                                <div class="flex gap-2 w-full justify-evenly">
                                                    <form action="{{ route('aboutcontent.delete', $ct->id) }}" method="POST" class="w-fit bg-red-600 p-2 rounded-md">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit">
                                                            Delete
